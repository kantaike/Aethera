...
export const CharacterListItem = ({ character }: { character: CharacterPreview }) => {
  ...
  const t = translations.features.characterListItem[language];
  let widthValue = '0%';
      ...
      <Card variant="blue" className={styles.card}>
        <div className={styles.portraitWrapper}>
          {character.art?.filePath ? (
            <img src={character.art.filePath} alt={character.name ?? t.portraitAlt} className={styles.avatar} />
          ) : (
            <div className={styles.avatarPlaceholder}>
              <span className={styles.placeholderIcon}>⚔</span>
              <span className={styles.placeholderText}>{t.noAvatar}</span>
            </div>
          )}
        ...
        <div className={styles.info}>
          ...
          {character.hp && (
            <div className={styles.hpBarContainer}>
              <div className={styles.hpBarFill} style={{ width: widthValue }}></div>
            </div>
          )}
        </div>
      </Card>
